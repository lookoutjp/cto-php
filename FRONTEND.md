...
| `/mypage` | `MypageController`（route 名 `dashboard`） | Mypage.asp | ログイン後の入口。本日の計画作業 / 管理タスク対応状況（todo・課題・リスク・WBS × 新規/接近/遅延/期限未設定）/ 定例作業対応状況。集計は `App\Support\TaskDashboard`。パンくず＋右サイドバー「MyMenu」（`layouts/partials/my-menu.blade.php`、旧ASP相当）の2カラム。会員トップナビ（`layouts/navigation.blade.php`）は機能一覧を出さず最小限（ホーム/ニュース/掲示板/MyPage/アップロード）にし、業務機能は MyMenu 側（プロジェクト参加者のみ、`TaskKind` 各種の起票/私の担当/全員のタスク、WBS/サーベイ/メンバー/メッセージ/ファイル、有効な機能のみ表示）に集約。現状 MyMenu は mypage 専用（他の会員ページはまだ旧来の全幅レイアウトのまま） |
